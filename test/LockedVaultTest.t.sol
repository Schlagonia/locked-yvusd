...
contract LockedVaultTest is Test {
    ...
    function test_deposit_compoundsIntoWrappedVault() public {
        ...
        assertGt(
            yvUSD.balanceOf(address(lockedVault)),
            0,
            "vault shares should be deployed"
        );
    }
    ...
    function test_onlyGovernanceCanUpdateCooldownConfig() public {
        ...
        assertEq(lockedVault.cooldownDuration(), 0, "cooldown should update");
        assertEq(
            lockedVault.withdrawalWindow(),
            2 days,
            "window should update"
        );
    }
    ...
    function test_processReportAccruesPendingLockerBonus() public {
        ...
        uint256 lockerSharesBefore = yvUSD.balanceOf(address(lockedVault));

        vm.prank(management);
        yvUSD.process_report(strategy);

        uint256 pending = accountant.pendingLockerShares(address(yvUSD));
        uint256 accountantShares = yvUSD.balanceOf(address(accountant));
        assertGt(pending, 0, "locker bonus should accrue");
        assertGt(
            accountant.claimableFeeShares(address(yvUSD)),
            0,
            "accountant should keep the non-locker fee shares"
        );
        ...
        assertEq(
            accountant.pendingLockerShares(address(yvUSD)),
            0,
            "pending bonus should clear"
        );
        assertEq(
            yvUSD.balanceOf(address(lockedVault)),
            lockerSharesBefore + pending,
            "locker should receive the bonus shares"
        );
        assertEq(
            yvUSD.balanceOf(address(accountant)),
            accountantShares - pending,
            "accountant should only keep its own fee shares"
        );
    }

    function test_processReportRefundsLossesFromIdleCapital() public {
        uint256 loss = TEST_AMOUNT / 5;
        uint256 expectedRefund = loss / 2;

        _setRefundRatio(5_000);
        _depositDirectlyIntoWrappedVault(attacker, TEST_AMOUNT);

        address strategy = _deployMockStrategyWithDebt(TEST_AMOUNT);
        deal(address(asset), strategy, TEST_AMOUNT - loss);
        deal(address(asset), address(accountant), expectedRefund);

        uint256 totalAssetsBefore = yvUSD.totalAssets();
        ...
        assertEq(
            yvUSD.totalAssets(),
            totalAssetsBefore - loss + expectedRefund,
            "idle capital should offset half the loss"
        );
        assertEq(
            asset.balanceOf(address(accountant)),
            0,
            "refund capital should be pulled into the vault"
        );
    }

    function test_processReportRefundsLossesFromReserveCapital() public {
        uint256 loss = TEST_AMOUNT / 5;
        uint256 expectedRefund = loss / 2;

        _setRefundRatio(5_000);

        IVault reserveVault = _deployReserveVault();
        vm.prank(GOVERNANCE);
        accountant.setReserveVault(address(yvUSD), address(reserveVault));

        deal(address(asset), address(accountant), expectedRefund);
        vm.prank(GOVERNANCE);
        accountant.deployIdleToReserve(address(yvUSD), expectedRefund);
        ...
        uint256 reserveSharesBefore = reserveVault.balanceOf(address(accountant));
        uint256 totalAssetsBefore = yvUSD.totalAssets();
        ...
        assertEq(
            yvUSD.totalAssets(),
            totalAssetsBefore - loss + expectedRefund,
            "reserve capital should offset half the loss"
        );
        assertLt(
            reserveVault.balanceOf(address(accountant)),
            reserveSharesBefore,
            "reserve shares should be unwound"
        );
        assertEq(
            asset.balanceOf(address(accountant)),
            0,
            "reserve withdrawal should end up inside the vault"
        );
    }
    ...
    function test_strategyReportHarvestsWrappedVaultProfit() public {
        ...
        assertGt(
            accountant.pendingLockerShares(address(yvUSD)),
            0,
            "report should have pending locker bonus to pull"
        );
        ...
        assertEq(
            accountant.pendingLockerShares(address(yvUSD)),
            0,
            "report should pull bonus shares before accounting"
        );
        assertGt(
            lockedVault.totalAssets(),
            totalAssetsBefore,
            "total assets should increase after harvest"
        );
    }

    function test_accountantHealthCheckRevertsUnexpectedProfit() public {
        vm.prank(management);
        lockedVault.setDoHealthCheck(true);
        vm.prank(management);
        lockedVault.setProfitLimitRatio(1);
    ...
    function test_accountantHealthCheckRevertsUnexpectedLoss() public {
        vm.prank(management);
        lockedVault.setDoHealthCheck(true);
        vm.prank(management);
        lockedVault.setLossLimitRatio(1);

        _depositDirectlyIntoWrappedVault(attacker, TEST_AMOUNT);

        address strategy = _deployMockStrategyWithDebt(TEST_AMOUNT);
        uint256 loss = TEST_AMOUNT / 10;
        deal(address(asset), strategy, TEST_AMOUNT - loss);
    ...
    function _setRefundRatio(uint16 refundRatio) internal {
        vm.prank(GOVERNANCE);
        accountant.setVaultConfig(
            address(yvUSD),
            address(lockedVault),
    ...
    function _deployReserveVault() internal returns (IVault reserveVault) {
        vm.startPrank(management);
        reserveVault = IVault(
            vaultFactory.deploy_new_vault(
                address(asset),
        ...
        reserveVault.set_role(management, Roles.ALL);
        reserveVault.set_deposit_limit(type(uint256).max);
        vm.stopPrank();
    }

    function _deployMockStrategyWithDebt(
        uint256 debtAmount
    ) internal returns (address) {
        Mock4626Strategy strategy = new Mock4626Strategy(
            asset,
            "Mock Strategy",
            "mSTRAT"
        );
    ...
    function _depositDirectlyIntoWrappedVault(
        address user,
        uint256 amount
    ) internal returns (uint256 shares) {
        vm.startPrank(user);
